...
function setup() {
    ...
    infectRateSlider.position(xPos, height - 60);
    recoveryDaysSlider = createSlider(1, 10, 4);
    recoveryDaysSlider.position(xPos, height - 30);
...
function draw() {
    ...
    fill(0);
    text('感染概率(0%~100%)', width / 2 + infectRateSlider.width / 2, height - 45);
    text('恢复天数(1~10d)', width / 2 + recoveryDaysSlider.width / 2, height - 15);
}
...
function spread() {
    ...
    for (let x = 1; x < columns - 1; x++) {
        for (let y = 1; y < rows - 1; y++) {
            next[x][y] = GOOD;
            // 如果我恢复了，就不会再改变
            if (board[x][y] == RECOVERD) next[x][y] = RECOVERD;
            // 如果我在活跃期，看看我是否该痊愈
